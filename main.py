...
def preload_member_names():
    ...
    try:
        ...
        print(f"✅ Preloaded {len(member_name_map)} member names")
    except Exception as e:
...
def find_closest_member_name(user_input: str):
    ...
    # Fallback: check DB with LIKE
    try:
        conn = connect_to_database()
        cursor = conn.cursor(dictionary=True)
        like_pattern = f"%{user_input.strip()}%"
        cursor.execute(
            "SELECT id, member_name FROM member_details WHERE member_name LIKE %s",
            (like_pattern,)
        )
        row = cursor.fetchone()
        conn.close()
        if row:
            return row['member_name'], row['id']
    except Exception as e:
        print(f"[ERROR] LIKE search failed: {e}")
...
- When filtering by member_name, ALWAYS use:
  LOWER(member_name) LIKE LOWER('%<search term>%')
- Use SELECT queries only
- Use JOIN if question involves both tables
- Never use '=' when filtering by member_name, always LIKE with wildcards
